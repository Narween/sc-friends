...
function getDb() {
  ...
  if (!columns.includes('location')) {
    // "Location" sur la page profil publique — optionnel, un citoyen peut
    // ne pas l'avoir renseigné (contrairement à "Enlisted", toujours
    // présent). Rempli par fetch-profiles.js, même page que enlisted_at.
    _db.exec(`ALTER TABLE friends ADD COLUMN location TEXT;`);
  }
